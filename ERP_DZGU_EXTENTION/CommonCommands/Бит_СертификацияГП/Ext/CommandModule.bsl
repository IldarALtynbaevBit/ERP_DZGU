...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
    Если ПараметрКоманды.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЭтаповПроизводства2_2.Завершен") Тогда 
        ...
        Для каждого стр из МассивСертификаций Цикл 
            ...
            ОткрытьФорму("Документ.БИТ_СертификацияНоменклатуры.ФормаОбъекта",ПараметрыФормы);
        КонецЦикла;
    Иначе
...
Функция ЗаполнитьСертификациюСрв(Ссылка)
    ...
    Для каждого стр из Ссылка.ВыходныеИзделия Цикл
